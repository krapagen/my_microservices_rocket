...
type config struct {
	...
	OrderPaid     orderPaidConsumerConfig     `yaml:"order_paid_consumer"`
	ShipAssembled shipAssembledProducerConfig `yaml:"ship_assembled_producer"`
...
func MustLoad(path string) {
	...
	if path != "" {
		...
		// Приоритет: env > yaml > env-default
		if err := cleanenv.ReadConfig(path, &cfg); err != nil {
			panic(fmt.Errorf("не удалось загрузить конфиг из %q: %w", path, err))
		}
		appConfig = &cfg
		return
	}
	...
	// Если путь не указан — читаем только из env-переменных
	if err := cleanenv.ReadEnv(&cfg); err != nil {
		panic(fmt.Errorf("не удалось загрузить конфиг из env: %w", err))
	}
	appConfig = &cfg
}
