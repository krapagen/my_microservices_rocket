...
// Release уменьшает зарезервированное количество деталей на 1 для каждого указанного UUID.
func (s *service) Release(ctx context.Context, uuids []uuid.UUID) error {
	op := "Функция inventory/internal/service/application/part/Release"
	log := slog.With("op", op)
	if len(uuids) == 0 {
	...
	for _, p := range parts {
		if p.Reserved() <= 0 {
			log.ErrorContext(ctx, "деталь %s не имеет зарезервированных единиц", "uuid", p.UUID())
			return fmt.Errorf("деталь %s не имеет зарезервированных единиц: %w", p.UUID(), errs.ErrNothingToRelease)
		}
